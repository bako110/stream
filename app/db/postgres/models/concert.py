...
# Données de stream (RTMP, HLS live, analytics) → MongoDB collection 'concert_streams'
...
class Concert(Base):
    ...
    artist_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    ...
    access_type: Mapped[AccessType] = mapped_column(Enum(AccessType), default=AccessType.free)
    status: Mapped[ConcertStatus] = mapped_column(Enum(ConcertStatus), default=ConcertStatus.draft)
