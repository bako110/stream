...
from app.db.postgres.models.concert import Concert, AccessType
...
class TicketService:
    ...
    async def get_ticket(ticket_id: uuid.UUID, user: User, db: AsyncSession) -> Ticket:
        ...
        if ticket.user_id != user.id and user.role.value != "admin":
    ...
    async def purchase_ticket(data: TicketCreate, user: User, payment_id: uuid.UUID | None, db: AsyncSession) -> Ticket:
        ...
        if concert.access_type not in (AccessType.ticket, AccessType.ppv):
            raise HTTPException(status_code=400, detail="Ce concert ne nécessite pas de billet")
        ticket = Ticket(
